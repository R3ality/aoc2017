...
while (index.this >= 0 && index.this < input.length) { // while in bounds
	...
	input[index.prev] = (input[index.prev] >= 3) ? input[index.prev]-1 : input[index.prev]+1; // modify instruction at previous index
	if (v) console.log(out +' thus next index is '+ index.this +' and instruction will be changed to '+ input[index.prev]);
	steps++; // increment the step counter
}

console.log('Steps in part II to reach the exit: '+ steps);
